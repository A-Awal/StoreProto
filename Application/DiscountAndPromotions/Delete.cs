...
namespace Application.DiscountAndPromotions
{
	public class Delete
	{
		...
		public class Handler : IRequestHandler<Command, Result<DiscountDto>>
		{
			...
			public async Task<Result<DiscountDto>> Handle(Command request, CancellationToken cancellationToken)
			{
				...
				foreach(Domain.Product product in _context.Products.Where(p => p.StoreId == discount.StoreId))
				{
