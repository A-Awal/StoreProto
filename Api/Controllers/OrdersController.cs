...
namespace API.Controllers
{
	public class OrdersController: BaseApiController
	{
		...
		public async Task<IActionResult> PuchaseAProduct(PurchaseCreateParam purchaseCreateParam)
		{
			...
			return HandleResult(
				await Mediator.Send(new CalculateOnPurchase.Command
				{
					OrderId = orderId.Value,
		...
		[HttpPut("UpdateCart")]
		public async Task<IActionResult> UpdatePurchase(PurchaseUpdateParam purchaseUpdateParam, Guid orderId)
		{
			return HandleResult(
				await Mediator.Send(
					new Update.Command
		...
		public async Task<IActionResult> Cart(Guid customerId)
		{
			...
			return HandleResult(
				await Mediator.Send(
					new CalculateTotlas.Query { OrderDto = dto.Value }
					)
			);
		}
		...
		{
			var dto = await Mediator.Send(
				new Cart.Query { CustomerId = customerId, Cart = false });
		...
		public async Task<IActionResult> StoreOrders(Guid storeId)
		{
			return HandleResult(
				await Mediator.Send(
					new GetCustomerOrders.Query { StoreId = storeId, Cart = false })
			);
		}
		...
		public async Task<IActionResult> StoreCart(Guid storeId)
		{
			return HandleResult(
				await Mediator.Send(
					new GetCustomerOrders.Query { StoreId = storeId, Cart = true })
			);
		}
		...
		public async Task<IActionResult> TagAsProcess(Guid orderId)
		{
			return HandleResult(
				await Mediator.Send(
					new TagProcessing.Command { OrderId =  orderId})
			);
		}
		...
		public async Task<IActionResult> TagAsShipping(Guid orderId)
		{
			return HandleResult(
				await Mediator.Send(
					new TagShipping.Command { OrderId =  orderId})
			);
		}
		...
		public async Task<IActionResult> TagAsDelivered(Guid orderId)
		{
			return HandleResult(
				await Mediator.Send(
					new TagDelivered.Command { OrderId =  orderId})
			);
		}
		...
		public async Task<IActionResult> TagAsCancelled(Guid orderId)
		{
			var cancellation = await Mediator.Send(
					new TagCancelled.Command { OrderId = orderId }
					);
