...
namespace API.Controllers
{
	public class ShippingController : BaseApiController
	{
		...
		public async Task<IActionResult> UpdateShippingDetail(UpdateParam updateParam, Guid shippingId)
		{
			return HandleResult(
				await Mediator.Send(
					new Application.Shipping.Update.Command { UpdateParam = updateParam, ShippingDetailId = shippingId }
					)
			);
		}
		...
		public async Task<IActionResult> GetCustomerShippingDetail( Guid CutomerId)
		{
			return HandleResult(
				await Mediator.Send(
					new GetShippingDetails.Query { CustomerId = CutomerId }
					)
			);
		}

		[HttpGet("GetCustomerStoreShippingDetail")]
		public async Task<IActionResult> GetCustomerStoreShippingDetail( Guid CutomerId, Guid storeId)
		{
			return HandleResult(
				await Mediator.Send(
		...
		public async Task<IActionResult> GetShippingDetail( Guid shippingDetailId)
		{
			return HandleResult(
				await Mediator.Send(
					new GetADetail.Query { ShippingDetailId = shippingDetailId })
			);
		}
		...
		public async Task<IActionResult> ApplyToAOrder(Guid orderId, Guid shippingDetailId)
		{
			return HandleResult(
				await Mediator.Send(
					new ApplyToOrder.Command { ShippingDetailId = shippingDetailId, OrderId = orderId }
					)
			);
		}
		...
		public async Task<IActionResult> RemoveFromOrder(Guid orderId, Guid shippingDetailId)
		{
			return HandleResult(
				await Mediator.Send(
					new RemoveFromOrder.Command { ShippingDetailId = shippingDetailId, OrderId = orderId }
					)
			);
		}
